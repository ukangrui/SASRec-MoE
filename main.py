...
if __name__ == '__main__':
    ...
    logging.info(f'base model ndcg: {ndcg}, ht: {ht}')
        ...
        for metric in config.metrics:
            ...
            for train_split in allsplits:
                ...
                if float(base_ndcg) > config.skip_threshold: ### skip well trained split
                    lora_model.save_pretrained(f'data/{str(args.dataset)}/lora/{str(metric)}/num_groups={config.num_groups}/{str(train_split)}/ndcg={round(base_ndcg,3)}')
                    ...
                    best_ndcg = base_ndcg
                    lora_model.save_pretrained(f'data/{str(args.dataset)}/lora/{str(metric)}/num_groups={config.num_groups}/{str(train_split)}/ndcg={round(best_ndcg,3)}')
                    ...
                    for epoch in range(config.lora.num_epochs):
                        ...
                        if (ndcg > best_ndcg):
                            ...
                            lora_model.save_pretrained(f'data/{args.dataset}/lora/{metric}/num_groups={config.num_groups}/{train_split}/ndcg={round(best_ndcg,3)}')
            ...
            print(f'base ndcg: {np.mean(list(base_metric.values()))}')
            print(f'lora ndcg: {np.mean(list(lora_metric.values()))}')
            sampler.close()
            clear_lora_cache()
        del model
        del sampler
        ...
        for metric in config.metrics:
            ...
            for epoch in range(config.moe.num_epochs):
                ...
                with torch.no_grad():
                    ...
                    if ndcg > best_ndcg:
                        best_ndcg = ndcg
                        torch.save(moe_model,f'data/{args.dataset}/checkpoint/{metric}_ndcg={round(best_ndcg,3)}.pt')
    ...
    checkpoints = list()
    for metric in config.metrics:
        max_value = max([float(i.split('=')[1].replace('.pt', '')) for i in os.listdir(f"data/{args.dataset}/checkpoint/") if metric in i])
        checkpoint_path = "data/" + args.dataset + '/checkpoint/' + metric + '_ndcg=' + str(round(max_value, 3)) + '.pt'
    ...
    for model in models:
        model.eval()
    ndcg, ht = evaluate_ensemble(models, dataset, config, alpha= list(config.ensemble.alpha))
    logging.info(f'Ensemble best ndcg: {ndcg}')