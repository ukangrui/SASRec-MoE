...
import argparse
from models.sasrec_moe_hard import SASRecMoEHard
import os
from models.sasrec_base import SASRec
import pickle
...
moe_model_hard = SASRecMoEHard(user_num = num_u, item_num = num_i, maxlen = 200, num_blocks = 2, num_heads = 1, hidden_units = 50, dropout_rate = 0.2, device = args.device, grouping_file=split_file)
moe_model_hard.load_state_dict(torch.load(f'checkpoints/{args.dataset}-base.pth', map_location=torch.device(args.device)), strict=False)
...
moe_model_hard = moe_model_hard.to(args.device)
moe_model_hard.eval()
ndcg, ht = 0, 0
with torch.no_grad():
    for test_batch in test_loader:
        u, seq, pos, test_items, mask = test_batch
        batch_ndcg, batch_ht = eval_step(moe_model_hard, u, seq, pos, test_items, mask, topK = args.topK)
        ndcg += batch_ndcg
        ht += batch_ht
